...
def robosuite_hyperparameters(config):
    ...
    config.algo.gmm.enabled = False                      # enable GMM policy - policy outputs GMM action distribution
    ...
    config.algo.rnn.horizon = 20        # unroll length for RNN - should usually match train.seq_length
    ...
    config.algo.rnn.open_loop = True   # if True, action predictions are only based on a single observation (not sequence) + hidden state
...
def get_config(dataset_type="robosuite", dataset_path=None, output_dir=None, debug=False):
    ...
    config.experiment.logging.terminal_output_to_txt = True    # whether to log stdout to txt file 
    ...
    if debug:
        ...
        config.experiment.rollout.horizon = 20
...
if __name__ == "__main__":
    ...
    config.experiment.name = f"Fixed_horizon_20_multirollout"
    # config.train.hdf5_filter_key = "50_percent_train"
    # config.train.hdf5_validation_filter_key = "50_percent_valid"
    
    # Training on discretized dataset
    discretize_ratio = 1
    
    # 1. Let the dataset to discretize the trajectory
    config.train.discretize_ratio = discretize_ratio
    # 2. Set the length of the RNN and seq_length to mean same time as non-discretized dataset
    config.algo.rnn.horizon = int(config.algo.rnn.horizon / discretize_ratio)

    # 3. Set whether to use multi-step rollout
    config.experiment.rollout.multi_step = True
    # 3. Set the rollout parameters (timesteps) to match the actual timestep.
    # It seems we don't need to do this "for now"
    # We only train the discretized datset and let the rollout to be not discretized
    # The total number of compuation cost will be reduced since the number of horizon for same amount of time will be reduced
