...
def algo_config_to_class(algo_config):
    ...
    if gaussian_enabled:
        ...
        if rnn_enabled:
            ...
            algo_class, algo_kwargs = BC_MultiStep, {}
...
class BC(PolicyAlgo):
    ...
    def process_batch_for_training(self, batch):
        ...
        print(f"BC batch: {batch['actions'].shape}")
        input()
...
class BC_MultiStep(PolicyAlgo):
    ...
    def _create_networks(self):
        ...
        self.nets["policy"] = PolicyNets.ActorNetwork(
            obs_shapes=self.obs_shapes,
            goal_shapes=self.goal_shapes,
            ac_dim=self.ac_dim*self.global_config.train.seq_length,
            mlp_layer_dims=self.algo_config.actor_layer_dims,
            encoder_kwargs=ObsUtils.obs_encoder_kwargs_from_config(self.obs_config.encoder),
        )
        self.nets = self.nets.float().to(self.device)
    ...
    def process_batch_for_training(self, batch):
        ...
        input_batch["actions"] = batch["actions"].reshape(batch_size, -1)
        # we move to device first before float conversion because image observation modalities will be uint8 -
        # this minimizes the amount of data transferred to GPU
        return TensorUtils.to_float(TensorUtils.to_device(input_batch, self.device))
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        """
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(BC_MultiStep, self).train_on_batch(batch, epoch, validate=validate)
            predictions = self._forward_training(batch)
            losses = self._compute_losses(predictions, batch)
    ...
    def log_info(self, info):
        ...
        Returns:
            loss_log (dict): name -> summary statistic
        """
        log = super(BC_MultiStep, self).log_info(info)
        log["Loss"] = info["losses"]["action_loss"].item()
        if "l2_loss" in info["losses"]:
            log["L2_Loss"] = info["losses"]["l2_loss"].item()
        if "l1_loss" in info["losses"]:
    ...
    def get_action(self, obs_dict, goal_dict=None):
        ...
        return self.nets["policy"](obs_dict, goal_dict=goal_dict).reshape(batch_size, -1, self.ac_dim)
