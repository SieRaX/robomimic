...
def robosuite_hyperparameters(config):
    ...
    config.experiment.rollout.horizon = 200                     # set horizon based on length of demonstrations (can be obtained with scripts/get_dataset_info.py)
    ...
    # rnn policy config
    config.algo.rnn.enabled = False      # enable RNN policy
    ...
    config.experiment.rollout.multi_step = False
    # Actor Layer dim
    config.algo.actor_layer_dims = [1024, 1024, 1024, 1024]
...
def get_config(dataset_type="robosuite", dataset_path=None, output_dir=None, debug=False):
    ...
    config.experiment.name = f"horizon_{config.train.seq_length}"   # name of experiment used to make log files
